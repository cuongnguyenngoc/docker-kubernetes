...
II. Kubernetes with Yaml, json file
  ...
  1. Create Pod
    ...
    ```kubectl create -f file_path_mysql_pod.yaml```
  2. Create Deployment
    ...
    spec:
      ...
      template: # this creates pods so the code is the same as when creating pod above
        metadata:
          labels:
            key1: value1
            key2: value2
  ...
  3. ReplicationController
    ...
    spec:
      replicas: number_of_pods
      selector: # this will select the pods which have label ```app=name_of_app```          # to be maintained by this ReplicationController
        app: name_of_app
